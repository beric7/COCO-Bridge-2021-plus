...
class concat:
    ...
    def concat_images(self, pad=False):
...
def concat_lists(image_list, directory_list, destination_dir, pad=False):
    if not os.path.exists(destination_dir): # if it doesn't exist already
        os.makedirs(destination_dir)
...
directory_list = [source_image_dir, source_image_dir, source_image_dir]
concat_lists(image_list, directory_list, './multi_concat/', pad=True)